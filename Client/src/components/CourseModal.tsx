...
export const CourseModal = ({ course }: { course: Course | any }) => {
  ...
  const addToCart = () => {
    ...
    if (cartCourse) {
      toast("Course đã trong giỏ hàng", { type: toast.TYPE.WARNING });
    } else {
      toast(
        `Course ${course.title} - ${numberToVietnameseDong(
          course?.price
        )} đã vào giỏ hàng`,
      ...
      <div className="w-full">
        ...
        <div className="text-center mt-10">
          {account?.rolename === "CUSTOMER" ? (
            myCourse.some((mc: any) => mc.course.courseId === course.courseId) ? (
